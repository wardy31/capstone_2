...
import { deleteCollege } from "../../../../store/actions/collegeAction";
import { setDialog } from "../../../../store/reducers/collegeReducer";
import store from "../../../../store/store";
import DeleteTwoToneIcon from "@mui/icons-material/DeleteTwoTone";

function DeleteModal({ id }) {
  const { dialog, loading } = useSelector((state) => state.college.delete);
  ...
  const handleForm = async () => {
    const res = store.dispatch(deleteCollege(id));
    if (res) {
      handleModal();
    }
  };
