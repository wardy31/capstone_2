...
import { deleteResearch } from "../../../store/actions/researchAction";
import { setDialog } from "../../../store/reducers/researchReducer";
import store from "../../../store/store";

function DeleteModal({ id }) {
  const { dialog, loading } = useSelector((state) => state.research.delete);
  ...
  const handleForm = async () => {
    const res = store.dispatch(deleteResearch(id));
    if (res) {
      handleModal();
    }
  };
